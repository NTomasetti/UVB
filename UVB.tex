...
\begin{algorithm}[H]
 ...
 \For{$n \mbox{ in } 1, \ldots, N-1$}{
   ...
   Use $q_{\lambda_{n}}(\theta | y_{1:T_{n}})$ and (\ref{scoreDeriv}) to choose the $\lambda_{n+1}$ that minimises the KL divergence from $q_{\lambda_{n+1}}(\theta | y_{1:T_{n+1}})$ to $\hat{p}(\theta | y_{1:T_{n+1}})$ 
  }
