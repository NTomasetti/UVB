...
// Get probabilities of K from probKHF, and insert the thetaD row and pS1 value associated with that draw into this function.
...
// [[Rcpp::export]]
mat forecastHF(vec y, vec x, vec thetaC, vec thetaD, vec fcVar, vec order, double pS1, vec rho, vec support, int H){
  int N = support.n_elem,  T = y.n_rows - H - 1;
  mat density (N, H, fill::zeros);
  
  vec ylag = {y(T), y(T-1), y(T-2)};
  double mu = thetaD(1), beta = thetaD(2);
  for(int h = 1; h <= H; ++h){
    pS1 = pS1 * (1 - rho(1)) + (1 - pS1) * rho(0);
    
    double mean = mu + beta * x(T + h);
    for(int i = 0; i < order.n_elem; ++i){
      if(i < 3){
        mean += thetaD(3 + i) * (ylag(i) - mu - beta * x(T + h - order(i)));
      } else {
        mean += thetaD(3 + i) * (y(T + h - order(i)) - mu - beta * x(T + h - order(i)));
      }
    }
    ...
    for(int i = 0; i < N; ++i){
      density(i, h-1) = pS1 / sqrt(2 * 3.14159 * exp(thetaC(0))) * exp(-pow(support(i) - thetaC(1), 2) / (2 * exp(thetaC(0)))) + 
        (1 - pS1) / sqrt(2 * 3.14159 * fcVar(h-1)) * exp(-pow(support(i) - mean, 2) / (2 * fcVar(h-1)));
    }
  }
  return density;
}
...
// priorMean: A matrix, first column: constant mean (pad with zeros), other columns dynamic mean 
// priorLinv: A cube, first slide: constant linv, other: dynamic linv
...
struct electricitySwitching {
  ...
  T operator ()(const Matrix<T, Dynamic, 1>& lambda)
    ...
    for(int i = 0; i < N; ++i){
      ...
      for(int t = tau; t < y.n_rows; ++t){
        T eta0 = 1.0 / sqrt(2 * 3.14159 * exp(thetaC(0))) * exp(-pow(y(t) - thetaC(1), 2) / (2 * exp(thetaC(0))));
